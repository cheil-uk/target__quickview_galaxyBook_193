    ...
    tagging(panelBtn,
      {
        ...
        "data-omni"       : `uk:426:pf:quickview:neo-qled:${panelLink.innerText}`,
        "ga-ca"           : "product click",
...
popUp(name, modelCode, variants, rating, features, price, promoPrice, image, benefits, exUrl, baseCode, proRating) {
  ...
  //checks to see there was or is another pop up modal, if there is then it removes it
  checkPopup();
  ...
  const viusalNavContainer = document.querySelector('.nv14-visual-lnb__container');
  const span = document.getElementsByClassName('close')[0];
  //seeting the visual nav z index so that my modal is on top
  // console.log(window.scrollY)
      span.onclick = () => {
        modal.style.display = 'none';
        viusalNavContainer.style.zIndex = 2999
      }
      window.onclick = (e) => {
      if (e.target === modal) {
        modal.style.display = 'none';
    ...
    (window.scrollY <= 180 ) ? viusalNavContainer.style.zIndex = 99 : viusalNavContainer.style.zIndex = 2999
